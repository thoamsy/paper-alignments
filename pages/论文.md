...
![image-20180421140902651](/Users/yk/Documents/paper/pages/image-20180421140902651.png)

这个图可能有点丑, 解释下: a 网页里包含了两个 b 和 c 网页. 当爬取完  a 后, 深度加一. 继续开始爬取 b 和 c 的网页, b 又有 e 和 f, c 网页有 h 和 i 网页. 就这样当 b 和 c 爬取完后, 深度继续加一. 就这样通过跟踪深度, 也能在合适的时机退出来. 
...
可是为了实现一个搜索引擎, 上面的这种方法就不行了, **它不够快**. 同时爬虫的编写并不是本文最重要的部门, 浪费太多笔墨来强调如果写出一个高性能的 nodejs 爬虫是不划算的. 于是这里要介绍 Scrapy[^1] 一个强大的 Python 爬虫框架来完成本文接下来的爬取任务. 让我们将重心放在其他的事情上面.
...
注意第二句, **yet extensible  way** 就能知道它满足了性能上的需求. 另外上面提到的爬虫需要注意的问题, 它已经全部抽象了. 使用者只需要专注于如何编写爬虫的逻辑, 不需要考虑其他的细节.



根据官方文档[^2], 可以很容易写出几个爬虫, 用来爬取特定的内容. 这里以 Stack Overflow 官网为例, 如果想知道今天的问题有哪些, 可以自己上浏览器输入, https://stackoverflow.com/?tab=featured. 也可以手动写一个爬虫, 来爬取关注的信息. 这里以获得每个问题的
...
就将所有爬取到的内容整理成一个 json 文件. 有了 JSON 文件, 相比每次搜索都立刻去爬取数据, 速度就能快很多!!
